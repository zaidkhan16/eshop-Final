...
router.post("/create-shop", catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    if (req.body.avatar && req.body.avatar !== "" && typeof req.body.avatar === "string" && (req.body.avatar.startsWith("data:") || req.body.avatar.startsWith("http"))) {
      try {
        myCloud = await cloudinary.v2.uploader.upload(req.body.avatar, {
          folder: "avatars",
        });
      } catch (cloudErr) {
        console.warn("Shop avatar upload fallback warning:", cloudErr.message);
      }
    }
    ...
    try {
      ...
      await sendMail({
        email: seller.email,
        subject: "Activate your Nexus Next-Gen Market Shop",
        message: `Hello ${seller.name}, welcome to Nexus Next-Gen Market Seller Portal! Please verify your email address to activate your shop.`,
        html: emailHtml,
      });
      res.status(201).json({
        success: true,
...
router.post(
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      if (existingSeller) {
        return next(new ErrorHandler("This shop account is already activated. Please sign in!", 400));
      }

      return next(new ErrorHandler("No pending shop registration found for this email. Please register your shop.", 404));
    } catch (error) {
