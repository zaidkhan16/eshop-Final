...
const SellerActivationPage = () => {
  ...
  const getCleanToken = () => {
    ...
    if (!pathToken && typeof window !== "undefined") {
      if (window.location.pathname.includes("/seller/activation/")) {
        pathToken = window.location.pathname.split("/seller/activation/")[1];
      } else if (window.location.pathname.includes("/activation/")) {
        pathToken = window.location.pathname.split("/activation/")[1];
      } else {
        const segments = window.location.pathname.split("/").filter(Boolean);
        if (
          segments.length > 0 &&
          segments[segments.length - 1] !== "activation" &&
          segments[segments.length - 1] !== "seller"
        ) {
  ...
  useEffect(() => {
    ...
    if (!resolvedToken) {
      setErrorMessage("Shop activation token is missing. Please check your email link or register your shop again.");
      setStatus("error");
      return;
    }
      ...
      const sendRequest = async () => {
        try {
          ...
          const msg =
            ...
            err.message ||
            "This seller activation link is invalid or has expired.";
          setErrorMessage(msg);
          setStatus("error");
        }
      };
    ...
    axios
      ...
      .catch((err) => {
        setErrorMessage(
          err.response?.data?.message ||
            err.message ||
            "Verification failed. Please register your shop again."
        );
        setStatus("error");
      });
  };
      ...
      <div className="w-full max-w-md bg-slate-900/80 backdrop-blur-2xl border border-slate-800 rounded-3xl p-8 shadow-2xl relative z-10 text-center">
        ...
        {status === "verifying" && (
          <div>
            ...
            <h2 className="text-2xl font-bold text-slate-100 mb-2">
              Verifying Shop Email
            </h2>
            <p className="text-slate-400 text-sm leading-relaxed mb-6">
              Please wait while we validate your shop activation token and initialize
        ...
        {status === "success" && (
          <div>
            ...
            <p className="text-slate-300 text-sm leading-relaxed mb-6">
              Your seller account has been verified successfully. You can now login
            ...
            <div className="bg-slate-800/80 border border-slate-700/60 rounded-xl p-3 mb-6 flex items-center justify-between text-xs text-slate-300">
              <span>Redirecting to Shop Dashboard...</span>
        ...
        {status === "error" && (
          <div>
            ...
            <p className="text-slate-400 text-sm leading-relaxed mb-6">
              {errorMessage ||
                "This seller activation link is invalid, expired, or has already been used."}
            </p>
            ...
            <div className="space-y-3">
              ...
              >
                <FiRefreshCw size={14} className="inline mr-1" /> Retry Shop Activation
              </button>
              ...
              >
                Already verified? Back to Shop Login
              </Link>
            </div>
          </div>
        )}
